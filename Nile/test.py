...
def test_shipping(f):
    ...
    if costs != "$1.04":
        print(
            "calculate_shipping_cost((0, 0), (1, 1)) returned {}. Expected result is {}"
            .format(costs, "$1.04"))
        return
...
def test_driver(f):
    ...
    if price != 1600:
        print(
            "calculate_driver_cost() did not provide correct final price (expected {}, received {})"
            .format(price, 1600))
        return
...
def test_money(f):
    try:
        ...
        print(
            "calculate_money_made() doesn't expect multiple trip keyword arguments"
        )
        return
